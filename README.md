...
| 3 | 1.0 kΩ 1% | Axial 1/4 W | VGA R3/G3/B3 MSB |
...
## 20. CPC Native Display — Cycle-Accurate Reconstruction

Standard CPC emulators get away with rendering one complete frame from RAM at frame-end. That is not sufficient for demos and games that use advanced display techniques. We need a proper **CRTC state machine** running in Core 1, synchronised to the Z80's actual bus activity as seen by Core 0.

### 20.1 What must be tracked

**CRTC registers — all 18, change can take effect mid-frame:**

| Register | Function | Critical timing |
|----------|---------|----------------|
| R0 | H total | Next line |
| R1 | H displayed | Next line |
| R2 | H sync position | Next line — Rapture effect |
| R3 | Sync widths | Next line |
| R4 | V total | Next frame |
| R5 | V adjust | Next frame |
| R6 | V displayed | Next frame — overscan |
| R7 | V sync position | Next frame |
| R9 | Max raster address | Immediately — line doubling, half-pixel scroll |
| R12/R13 | Screen start address | Immediately — split screen |

**Gate Array — changes take effect at next pixel clock edge (sub-scanline):**

- Palette entries (ink 0–15 in mode 0, inks 0–3 in mode 1, inks 0–1 in mode 2)
- Border colour
- Screen mode (0, 1 or 2) — can switch mid-scanline, changing pixel width

**Gate Array interrupt counter:**

- Increments on every CRTC scanline
- Resets to 0 on interrupt acknowledge (INTACK — /IORQ + /M1 both low)
- Fires /INT when counter reaches 52
- Demos count interrupts and write palette changes at exact cycle offsets after each interrupt

### 20.2 CRTC state machine — Core 1

Core 1 runs a software model of the 6845 CRTC. It advances every 1 µs (the CRTC character clock = 16 MHz ÷ 16). This is the foundation everything else is built on.

```c
// CRTC state — updated every 1 µs by Core 1 DMA IRQ
typedef struct {
    uint8_t  hcc;          // horizontal character counter (0..R0)
    uint8_t  vcc;          // vertical character counter  (0..R4)
    uint8_t  raster;       // raster within character row (0..R9)
    uint8_t  vsc;          // vertical sync counter
    uint8_t  hsc;          // horizontal sync counter
    bool     hsync;        // HSYNC active
    bool     vsync;        // VSYNC active
    bool     display_en;   // display enabled (not in border or blanking)
    uint16_t ma;           // current memory address (from R12/R13 + counters)
    uint8_t  ga_int_count; // GA interrupt counter (0..51)
    uint8_t  regs[18];     // shadow of all 18 CRTC registers
} crtc_state_t;

// Advance by one character clock (called from 1 MHz timer or DMA IRQ)
void crtc_tick(crtc_state_t *s) {
    ...
    if (s->hcc > s->regs[0]) {         // end of line
        ...
        if (s->ga_int_count >= 52) {
            s->ga_int_count = 0;
            assert_int_to_cpc();        // drive /INT low for 1 µs
        }
        if (s->raster > s->regs[9]) {  // end of character row
            s->raster = 0;
            s->vcc++;
            if (s->vcc > s->regs[4])   // end of frame
                s->vcc = 0;
        }
    }
    // Update display_en, hsync, vsync, ma from register state
    // ...
}
```

### 20.3 Sub-scanline GA write tagging

Every Gate Array write captured by Core 0 is tagged with the current CRTC state machine position before being queued to Core 1. This gives us **character-clock resolution** (1 µs) for when each palette or mode change takes effect.

```c
typedef struct {
    uint8_t  ga_data;      // the byte written to the Gate Array
    uint8_t  scanline;     // vcc * (R9+1) + raster at time of write
    uint8_t  hcc;          // horizontal character position at time of write
} ga_event_t;

// Core 0 — on every GA write (&7Fxx):
void handle_gate_array_write(uint8_t data) {
    ga_event_t ev = {
        .ga_data  = data,
        .scanline = crtc.vcc * (crtc.regs[9] + 1) + crtc.raster,
        .hcc      = crtc.hcc,
    };
    ring_push(ga_event_ring, &ev);
}
```

Core 1 dequeues these events and applies them at exactly the right position during scanline rendering.

### 20.4 Interrupt acknowledge (INTACK) approximation

We do not have /IORQ or /M1 connected — those pins are used for RGB555 colour output. We cannot detect the INTACK cycle directly.

**What INTACK does on real hardware:** When the Z80 acknowledges the interrupt (/IORQ + /M1 both low simultaneously), the GA resets its scanline interrupt counter to 0. Demos use this to control exactly which scanlines produce palette changes — they deliberately delay INTACK with `DI` / `EI` manipulation to shift the counter phase.

**Our approximation:** We know precisely when our CRTC state machine fires /INT (every 52 scanlines). The Z80 acknowledges within a bounded window — typically 2–6 machine cycles (0.5–1.5 µs) after /INT is asserted, depending on what instruction it was executing. We model INTACK as occurring **1 µs after /INT assertion** and reset our GA interrupt counter at that point.

```c
void assert_int_to_cpc(void) {
    gpio_put(PIN_INT, 0);             // assert /INT low
    int_fire_scanline = current_scanline;
    int_pending = true;
}

// Called from CRTC tick, 4 character clocks (~1 µs) after /INT:
void model_intack(void) {
    if (int_pending) {
        crtc.ga_int_count = 0;        // approximate INTACK
        gpio_put(PIN_INT, 1);         // release /INT
        int_pending = false;
    }
}
```

**Accuracy:** For the vast majority of games and demos this is correct. Effects that rely on precise INTACK timing (e.g. deliberately executing slow instructions to delay acknowledgement and shift the counter phase) may be off by ±1 scanline. This is a known limitation of the design — restoring /IORQ and /M1 at the cost of two colour bits would fix it if needed in a future revision.

### 20.5 Overscan and full-frame rendering

Standard CPC: 40 chars × 25 rows = 320×200 active pixels, surrounded by border.
Overscan: R6 > 25 and/or R1 > 40, extending active display into the border region.
Maximum: ~48 chars wide and ~36 rows tall = up to ~768×288 pixels.

Core 1 renders the full CRTC addressable area, not just the standard 200 lines. The VGA output must cover this. In CPC native mode we output at 50 Hz (not 60 Hz — see section 20.6) with a frame tall enough to contain maximum overscan output.

```c
void render_frame(void) {
    for (int vcc = 0; vcc <= crtc.regs[4]; vcc++) {
        for (int raster = 0; raster <= crtc.regs[9]; raster++) {
            render_scanline(vcc, raster);
        }
    }
}

void render_scanline(int vcc, int raster) {
    bool in_display = (vcc < crtc.regs[6]);
    for (int hcc = 0; hcc <= crtc.regs[0]; hcc++) {
        // Apply any GA events queued for this position
        apply_ga_events_at(vcc * (crtc.regs[9]+1) + raster, hcc);

        if (in_display && hcc < crtc.regs[1]) {
            // Active display — decode from VRAM using current MA
            uint16_t addr = current_ma | (raster & 0x07) << 11;
            uint8_t  byte = emulated_ram[addr & 0x3FFF | 0x4000];
            decode_pixel(byte, ga_mode, ga_palette, line_buf, hcc);
        } else {
            // Border — fill with current border colour
            fill_border(line_buf, hcc, ga_border_colour);
        }
        advance_ma(hcc);
    }
}
...
### 20.6 VGA output at 50 Hz for CPC native mode

CPC outputs at 50 Hz PAL. Outputting at 60 Hz introduces a 10 Hz frame rate mismatch — every ~5 frames a CPC frame is either skipped or repeated, causing visible judder in smooth scrolling demos.

Solution: **CPC native mode outputs at 50 Hz**. Most VGA monitors manufactured after 2000 accept 50 Hz input. The PLL_USB is configured for the appropriate pixel clock:

```
CPC native VGA output: 832 × 312 total pixels at 50 Hz
Pixel clock = 832 × 312 × 50 = 12,979,200 Hz ≈ 13.0 MHz
PLL_USB: fbdiv=65, pd1=3, pd2=5 → VCO=780 MHz → 780/15 = 52.0 MHz ÷ 4 = 13.0 MHz  ✓  exact
```

At 13.0 MHz pixel clock with 832 × 312 totals, the display covers the full CPC raster including maximum overscan (up to 768 active pixels wide, up to 288 active lines). For modes narrower than the full width, unused pixels are filled with the border colour.

If the monitor does not support 50 Hz, an alternative 60 Hz mode repeats every 6th frame to compensate — acceptable for most games, but demos requiring exact raster timing should use 50 Hz mode.

### 20.7 Rapture and R2 mid-frame changes

The "Rapture" effect (and similar techniques) change R2 (H sync position) on specific scanlines to shift the monitor's sync relationship and produce colour distortion effects in the border region. Since we reconstruct rather than capture the CPC video signal, we handle this by:

1. Tracking R2 changes in the CRTC state machine with scanline accuracy
2. Adjusting the HSYNC position in our PIO sync generator on a per-line basis
3. Rendering border pixels at the shifted positions

The PIO sync generator accepts per-line parameters fed by DMA — each line's HSYNC position and width are independently configurable, allowing full Rapture effect reproduction.

### 20.8 Summary — what this achieves

| Technique | Supported? | Notes |
|-----------|-----------|-------|
| Overscan | ✓ Full | Up to 768×288 pixels rendered |
| Raster bars (border colour changes) | ✓ Full | 1 µs character clock resolution |
| Mid-frame palette changes | ✓ Full | Tagged with CRTC position |
| Mid-scanline mode switch | ✓ Full | Sub-character resolution |
| Split screen (R12/R13 mid-frame) | ✓ Full | Applied immediately |
| Rapture / R2 mid-frame changes | ✓ Full | Per-line HSYNC position |
| Line doubling (R9 changes) | ✓ Full | Applied immediately |
| Interrupt-timed effects | ✓ Approx | INTACK modelled as 1 µs after /INT — ±1 scanline accuracy |
| 50 Hz frame rate | ✓ Full | Native CPC rate — no frame skipping |
| Hardware sprites | N/A | CPC has none — not needed |
